...
function renderPanel(panelId) {
    ...
    const renderers = {
        ...
        'legacy-verification': renderLegacyVerification,
        'project-library': renderProjectLibrary,
    };
    ...
    if (renderer) {
        ...
        if (panelId === 'project-library') loadProjectLibrary();
    }
}
...
function autoGenerateInput(inputId, moduleName) {
    const el = document.getElementById(inputId);
    if(el) {
        el.value = `Please execute the ${moduleName} module. Analyze all established project context, market research, and independent verification audits. Automatically determine the optimal direction and generate a comprehensive, world-class outcome that maximizes our strategic advantage.`;
    }
}
        ...
        <div class="card mb-24">
            <div class="input-group">
                <label class="input-label" style="display:flex;justify-content:space-between;align-items:center">
                    <span>Describe the Company, Product, or Market to Analyze</span>
                    <button class="btn-sm btn-ghost" style="margin:0" onclick="autoGenerateInput('marketInput', 'Market Research')">✨ Auto-Generate</button>
                </label>
...
function renderBrandStudio() {
    ...
    return `
        ...
        <div class="card mb-24">
            <div class="input-group">
                <label class="input-label" style="display:flex;justify-content:space-between;align-items:center">
                    <span>Describe the Company or Product for Brand Identity</span>
                    <button class="btn-sm btn-ghost" style="margin:0" onclick="autoGenerateInput('brandInput', 'Brand Studio')">✨ Auto-Generate</button>
                </label>
...
function renderGTMPlanner() {
    ...
    return `
        ...
        <div class="card mb-24">
            <div class="input-group">
                <label class="input-label" style="display:flex;justify-content:space-between;align-items:center">
                    <span>Describe the Product and Target Market</span>
                    <button class="btn-sm btn-ghost" style="margin:0" onclick="autoGenerateInput('gtmInput', 'GTM Plan')">✨ Auto-Generate</button>
                </label>
...
function renderPersonaBuilder() {
    ...
    return `
        ...
        <div class="card mb-24">
            <div class="input-group">
                <label class="input-label" style="display:flex;justify-content:space-between;align-items:center">
                    <span>Describe the Product and Target Audience</span>
                    <button class="btn-sm btn-ghost" style="margin:0" onclick="autoGenerateInput('personaInput', 'Persona Builder')">✨ Auto-Generate</button>
                </label>
...
function renderCampaignHub() {
    ...
    return `
        ...
        <div class="card mb-24">
            <div class="input-group">
                <label class="input-label" style="display:flex;justify-content:space-between;align-items:center">
                    <span>Describe the Campaign Objective</span>
                    <button class="btn-sm btn-ghost" style="margin:0" onclick="autoGenerateInput('campaignInput', 'Campaign Hub')">✨ Auto-Generate</button>
                </label>
...
function renderCompetitiveIntel() {
    ...
    return `
        ...
        <div class="card mb-24">
            <div class="input-group">
                <label class="input-label" style="display:flex;justify-content:space-between;align-items:center">
                    <span>Describe the Company and Its Competitive Context</span>
                    <button class="btn-sm btn-ghost" style="margin:0" onclick="autoGenerateInput('competitiveInput', 'Competitive Intel')">✨ Auto-Generate</button>
                </label>
...
// ══════════════════════════════════════════════════════════
//  LEGACY VERIFICATION PANEL
// ══════════════════════════════════════════════════════════

function renderLegacyVerification() {
    const prev = state.results.legacy_audit;
    return `
        <div class="panel-header">
            <div class="panel-badge">Independent Audit</div>
            <h1 class="panel-title">Legacy Verification</h1>
            <p class="panel-subtitle">Upload or paste existing documentation. The CMO will independently audit it against its own findings to verify your assumptions and identify gaps.</p>
        </div>

        <div class="card mb-24">
            <div class="input-group">
                <label class="input-label" style="display:flex;justify-content:space-between;align-items:center">
                    <span>Paste Legacy Documentation</span>
                    <label class="btn-sm btn-ghost" style="cursor:pointer;margin:0">
                        📄 Load File
                        <input type="file" id="legacyFileInput" accept=".txt,.md,.json,.csv" style="display:none" onchange="loadLegacyFile(event)">
                    </label>
                </label>
                <textarea class="input-field" id="legacyInput" rows="8" maxlength="100000" placeholder="Paste business plans, legacy personas, old GTM strategies, or unstructured context here..."></textarea>
                <p class="text-xs text-dim mt-8" style="line-height: 1.4">Supported formats: .txt, .md, .csv, .json. For PDFs or Word documents, please copy and paste the text above. Max size: 2MB or 100,000 characters.</p>
            </div>
            <button class="btn btn-primary" onclick="runLegacyVerification()">🔍 Run Independent Verification</button>
        </div>

        <div id="legacyResults">${prev ? renderLegacyAuditResults(prev) : ''}</div>
    `;
}

function loadLegacyFile(event) {
    const file = event.target.files[0];
    if (!file) return;
    
    // Safeguard: 2MB limit
    if (file.size > 2 * 1024 * 1024) {
        showToast('File exceeds 2MB limit. Please truncate your data.', 'error');
    ...
    reader.onload = function(e) {
        document.getElementById('legacyInput').value = e.target.result;
        showToast('File loaded successfully', 'success');
    };
    reader.readAsText(file);
}

async function runLegacyVerification() {
    const legacyText = document.getElementById('legacyInput').value.trim();
    if (!legacyText) return showToast('Please provide legacy documentation to verify', 'error');
    if (legacyText.length > 100000) return showToast('Documentation exceeds 100,000 character limit.', 'error');
    
    const overlay = document.getElementById('loadingOverlay');
    if (overlay) {
        document.getElementById('loadingTitle').textContent = 'Auditing Legacy Context...';
        document.getElementById('loadingSub').textContent = 'Comparing against pristine CMO findings';
        overlay.classList.remove('hidden');
    }

    try {
        const r = await fetch(`${API}/legacy-audit`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
        ...
        if (data.error) {
            ...
            state.results.legacy_audit = data;
            document.getElementById('legacyResults').innerHTML = renderLegacyAuditResults(data);
        }
    } catch (e) {
        if (overlay) overlay.classList.add('hidden');
        showToast('Network error running verification', 'error');
        console.error(e);
    }
}

function renderLegacyAuditResults(d) {
    let html = `<div class="result-section">`;

    html += `<div class="callout insight"><div class="callout-title">📋 Verification Summary</div>${escapeHtml(d.audit_summary || '')}</div>`;

    // Flawed Assumptions
    if (d.flawed_assumptions && d.flawed_assumptions.length > 0) {
        html += `<div class="result-section-title mt-32"><span class="section-icon">⚠️</span> Flawed Assumptions Detected</div>`;
        html += `<div class="grid-2">`;
        d.flawed_assumptions.forEach(f => {
            html += `<div class="card" style="border-left:4px solid var(--red)">
                <div class="flex justify-between items-center mb-8">
                    <strong class="text-sm" style="color:var(--text-primary)">Client Assumption</strong>
                    ${severityTag(f.severity)}
                </div>
                <p class="text-sm text-dim mb-16" style="font-style:italic">"${escapeHtml(f.client_assumption)}"</p>
                <strong class="text-sm text-accent">CMO Correction</strong>
                <p class="text-sm mt-8">${escapeHtml(f.cmo_correction)}</p>
            </div>`;
        });
        html += `</div>`;
    }
    ...
    // Validated Context
    if (d.validated_context && d.validated_context.length > 0) {
        html += `<div class="result-section-title mt-32"><span class="section-icon">✅</span> Validated Historical Context</div>`;
        html += `<div class="card"><ul style="padding-left:20px;margin:0;color:var(--text-primary)">`;
        d.validated_context.forEach(v => {
            html += `<li style="margin-bottom:8px;font-size:14px">${escapeHtml(v)}</li>`;
        });
        html += `</ul></div>`;
    }

    // Strategic Reconciliation
    if (d.strategic_reconciliation) {
        html += `<div class="result-section-title mt-32"><span class="section-icon">🧭</span> Strategic Reconciliation</div>`;
        html += `<div class="card card-glow" style="padding:24px"><p style="font-size:15px;line-height:1.6">${escapeHtml(d.strategic_reconciliation)}</p></div>`;
    }
...
async function switchProject(name, displayName) {
    ...
    const overlay = document.getElementById('loadingOverlay');
    if (overlay) {
        document.getElementById('loadingTitle').textContent = 'Loading Project...';
        document.getElementById('loadingSub').textContent = `Hydrating ${escapeHtml(displayName || name)}`;
        overlay.classList.remove('hidden');
    }

    // Fetch project history to hydrate state
    try {
        const r = await fetch(`${API}/projects/${encodeURIComponent(name)}`);
        if (r.ok) {
            const detail = await r.json();
            
            // Safeguard: User switched projects while fetching
            ...
            if (detail.history && Array.isArray(detail.history)) {
                // history is ordered ASC, so iterating through it leaves the latest in state
                for (const entry of detail.history) {
                    const mod = entry.module;
                    const res = entry.result;
                    if (!res) continue;
                    
                    if (mod === 'market_research') state.results.market = res;
                    else if (mod === 'brand_studio' || mod === 'brand_refinement') state.results.brand = res;
                    else if (mod === 'gtm_plan') state.results.gtm = res;
                    else if (mod === 'personas') state.results.personas = res;
                    else if (mod === 'campaign') state.results.campaign = res;
                    else if (mod === 'competitive_analysis') state.results.competitive = res;
                    else if (mod === 'brand_visualize') state.brandVisual = res;
                    else if (mod === 'brand_critique') state.brandCritique = res;
                    else if (mod === 'legacy_audit') state.results.legacy_audit = res;
                }
            }
        } else {
            console.error("Failed to load project history", r.status);
            showToast('Failed to load project history', 'error');
        }
    } catch (e) {
        console.error("Failed to load project history", e);
        showToast('Network error loading project history', 'error');
    }
    ...
    // Render the active panel (or default to dashboard if on projects view)
...
function renderProjectLibrary() {
    return `
        <div class="panel-header">
            <div class="panel-badge">Asset Management</div>
            <h1 class="panel-title">Project Library</h1>
            <p class="panel-subtitle">Access all generated assets, visuals, and documents for the current project natively.</p>
        </div>
        <div id="libraryContainer">
            <div class="card"><div style="text-align:center;padding:40px;color:var(--text-dim)">Loading assets...</div></div>
        </div>
    `;
}
...
async function loadProjectLibrary() {
    try {
        const SERVER_BASE = "http://localhost:5020";
        const r = await fetch(`${API}/projects/${encodeURIComponent(state.projectName)}/assets`);
        const d = await r.json();
        const container = document.getElementById('libraryContainer');
        
        if (d.error) {
            container.innerHTML = `<div class="card"><div style="text-align:center;padding:40px;color:var(--red)">Error: ${escapeHtml(d.error)}</div></div>`;
            return;
        }
        
        const assets = d.assets || [];
        if (assets.length === 0) {
            container.innerHTML = `<div class="card"><div style="text-align:center;padding:40px;color:var(--text-dim)">No assets found for this project yet. Use the Brand Studio to generate some!</div></div>`;
            return;
        }

        let html = '<div class="grid-3">';
        assets.forEach(asset => {
            const fullUrl = `${SERVER_BASE}${asset.url}`;
            const isImage = asset.type === 'image';
            
            html += `
                <div class="card" style="padding:16px;display:flex;flex-direction:column;gap:16px;">
                    <div style="height:180px;border-radius:8px;overflow:hidden;background:var(--bg-card-hover);display:flex;align-items:center;justify-content:center;">
                        ${isImage ? `<img src="${fullUrl}" alt="${escapeHtml(asset.name)}" style="width:100%;height:100%;object-fit:cover;">` : `<span style="font-size:48px;">📄</span>`}
                    </div>
                    <div>
                        <h4 style="color:var(--text-primary);font-size:14px;margin-bottom:8px;word-break:break-all;">${escapeHtml(asset.name)}</h4>
                        <div style="display:flex;justify-content:space-between;align-items:center;">
                            <span class="tag tag-blue" style="text-transform:uppercase;">${escapeHtml(asset.type)}</span>
                            <a href="${fullUrl}" download="${escapeHtml(asset.name)}" target="_blank" style="color:var(--green);font-size:14px;text-decoration:none;display:flex;align-items:center;gap:4px;">
                                ⬇ Download
                            </a>
                        </div>
                    </div>
        ...
        document.getElementById('libraryContainer').innerHTML = `<div class="card"><div style="text-align:center;padding:40px;color:var(--red)">Failed to load assets: ${e.message}</div></div>`;
    }
}
