...
Generate a new, improved version of the brand visual."""
        ...
        for part in response.parts:
            if part.text is not None:
                ...
                try:
                    import sys
                    SYSTEM_CORE_PATH = Path(__file__).resolve().parent.parent.parent.parent.parent.parent / ".system_core"
                    if str(SYSTEM_CORE_PATH) not in sys.path:
                        sys.path.append(str(SYSTEM_CORE_PATH))
                    from project_manager import write_project_file
                    
                    file_path = write_project_file(project_name, "cmo", filename, image_data, caller_role="cmo")
                    image_url = f"/project-files/{project_name}/cmo/{filename}"
                except Exception as e:
                    print(f"[brand_visual] Failed to save using project_manager: {e}")
                    # Fallback
