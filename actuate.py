...
target_path = "apps/CLO_Agent/legal_engine.py"

# Pre-flight Request
qa_data = json.dumps({"target_path": target_path, "content": ast_payload}).encode('utf-8')
req = urllib.request.Request("http://127.0.0.1:8000/api/v1/qa/engine/pre-flight", data=qa_data, headers={'Content-Type': 'application/json'})
try:
    with urllib.request.urlopen(req) as response:
        print("PREFLIGHT RESPONSE:", response.read().decode())
except urllib.error.HTTPError as e:
    print("PREFLIGHT ERROR:", e.read().decode())

# Mutate Request
mutate_data = json.dumps({"relative_path": target_path, "content": ast_payload}).encode('utf-8')
req = urllib.request.Request("http://127.0.0.1:8000/api/v1/atomizer/mutate", data=mutate_data, headers={'Content-Type': 'application/json'})
try:
    with urllib.request.urlopen(req) as response:
        print("MUTATE RESPONSE:", response.read().decode())
except urllib.error.HTTPError as e:
    print("MUTATE ERROR:", e.read().decode())
