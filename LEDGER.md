...
- **Protocol:** Bifurcated Enterprise OS Doctrines
- **Doctrines:**
  1. **SENSOR MATRIX ISOLATION (LAYER 2):** All live internet intelligence gathering must strictly route through shared_modules/deep_research_crawler.py utilizing asynchronous aiohttp. The use of heavy headless browsers (e.g., Playwright or Antigravity ABC) is permanently forbidden for raw data ingestion; they are strictly reserved for QA and DOM validation. The CIO Agent acts as the sole gatekeeper for pre-flight intelligence injection.
  2. **LLM BIFURCATION & DEGRADATION PROTOCOL:** The ecosystem utilizes a strict dual-model architecture. Gemini 2.5 Pro/Flash is the default cognitive engine for strategic, creative, and multimodal tasks. Claude 3.7 Sonnet is strictly reserved for the CFO and CRITIC nodes requiring deep logical rigor. Silent try/except fallbacks within model_router.py are structurally forbidden; API key absence or network failures must trigger immediate terminal warnings and halt autonomous execution.
  3. **THE BIFURCATION HANDSHAKE (MODE B -> MODE A):** The Venture Architect (C-Suite) and the Technical Architect (IDE) must never communicate via unstructured conversational English. All business-logic-to-code execution handoffs must be serialized through a strict Infrastructure_Blueprint.json schema generated by the CTO node. This JSON acts as the absolute immutable contract for physical disk mutations.
