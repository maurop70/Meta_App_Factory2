from playwright.sync_api import sync_playwright
import time

def actuate_and_capture():
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True)
        page = browser.new_page(viewport={"width": 1280, "height": 800})
        
        page.goto("http://localhost:5175/admin")
        page.wait_for_load_state("networkidle")
        
        if "login" in page.url:
            page.fill("input[name='mwo_operator_id']", "ERP-1000")
            page.fill("input[name='mwo_operator_pin']", "1234")
            page.click("button:has-text('Authorize Session')")
            page.wait_for_url("**/admin")
            
        time.sleep(2)
        page.click("text=Procurement")
        time.sleep(3)
        
        # Click VIEW PAYLOAD
        page.click("button:has-text('VIEW PAYLOAD')")
        time.sleep(2)
        
        # Select FULFILLED using proper Playwright select_option
        page.select_option("select", value="FULFILLED")
        time.sleep(1)
        
        # Click EXECUTE STATE SHIFT
        page.click("button:has-text('EXECUTE STATE SHIFT')")
        
        # Wait for the modal to close and state to digest
        page.wait_for_selector("text=Authorize Procurement", state="hidden", timeout=10000)
        time.sleep(1)
        
        # Wait for the table to reflect FULFILLED status
        page.wait_for_selector("text=FULFILLED", timeout=10000)
        
        page.screenshot(path="C:\\Users\\mpetr\\.gemini\\antigravity\\brain\\3ebb2f72-cd27-4713-ac15-e156465c3747\\final_ui_resolution.png")
        print("Captured final_ui_resolution.png")
        browser.close()

if __name__ == "__main__":
    actuate_and_capture()
