...
class UIDesigner:
    ...
    def build_ui(self, app_path, app_name, port=5005):
        ...
        # ── 1. BACKEND: server.py ─────────────────────────────
        self._write_backend(app_path, app_name, port=port)
        ...
        self._write_app_jsx(src_dir, app_name, port=port)
    ...
    def _write_backend(self, app_path, app_name, port=5005):
        # Load portable template and substitute placeholders
        template_path = os.path.join(self._factory_dir, "templates", "server_template.py")
        if os.path.exists(template_path):
            with open(template_path, "r", encoding="utf-8") as f:
                code = f.read()
            code = code.replace("__APP_NAME__", app_name).replace("__PORT__", str(port))
            with open(os.path.join(app_path, "server.py"), "w", encoding="utf-8") as f:
                f.write(code)
            return
        # Fallback: inline template (legacy)
        code = f'''"""
...
# ── Import DocumentParserService ─────────────────────────
_FACTORY = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
...
@app.get("/")
def root():
    return {{"service": "{app_name}", "version": "3.0", "streaming": STREAMING, "parser": PARSER_READY}}
...
async def upload_document(file: UploadFile = File(...)):
    if not PARSER_READY:
        return JSONResponse({{"error": "Parser not available."}}, status_code=503)
    ...
    with open(dest, "wb") as f:
        ...
        f.write(content)
    result = _parser.parse(dest, source_app="{app_name}")
    if result.get("status") == "parsed":
        result = _router.route(result)
        _parser.log_to_master_index(result)
    return result

@app.get("/api/health")
def health():
    return {{"status": "healthy", "streaming": STREAMING, "parser": PARSER_READY}}
